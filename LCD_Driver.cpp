...
{
	LCD_WriteReg(0x2A);
	LCD_WriteData(Xpoint >> 8);	 		//Set the horizontal starting point to the high octet
	LCD_WriteData(Xpoint & 0xff);	 	//Set the horizontal starting point to the low octet
	LCD_WriteData((Xpoint - 1) >> 8);		//Set the horizontal end to the high octet
	LCD_WriteData((Xpoint - 1) & 0xff);	//Set the horizontal end to the low octet
	//set the Y coordinates
	LCD_WriteReg(0x2B);
	LCD_WriteData(Ypoint >> 8);
	LCD_WriteData(Ypoint & 0xff );
	LCD_WriteData((Ypoint - 1) >> 8);
	LCD_WriteData((Ypoint - 1) & 0xff);
  LCD_WriteReg(0x2C);   
}
