  ...
  if(DOT_STYLE == DOT_STYLE_DFT) {
    for(XDir_Num = 0; XDir_Num < 2 * Dot_Pixel - 1; XDir_Num++) {
      for(YDir_Num = 0; YDir_Num < 2 * Dot_Pixel - 1; YDir_Num++) {
        ...
        LCD_SetCursor((Xpoint + XDir_Num - Dot_Pixel), (Ypoint + YDir_Num - Dot_Pixel));
        LCD_WriteData(Color);
      }
    }
  } else {
                 ...
                 sFONT* Font, COLOR Color_Background, COLOR Color_Foreground)
{
    ...
    for(Page = 0; Page < Font->Height; Page ++ ) {
      for(Column = 0; Column < Font->Width; Column ++ ) {
        ...
        // if (Color_Background == FONT_BACKGROUND ) { //this process is to speed up the scan
        //   if (*ptr & (0x80 >> (Column % 8)))
        //     GUI_DrawPoint(Xpoint + Column, Ypoint + Page, Color_Foreground, DOT_PIXEL_DFT, DOT_STYLE_DFT);
        // } else 
        {
          LCD_SetCursor((Xpoint + Column), (Ypoint + Page));
          if (*ptr & (0x80 >> (Column % 8))) {
            LCD_WriteData(Color_Foreground );
          } else {
            LCD_WriteData(Color_Background );
          } /* end if */
        } /* end if */
        //One pixel is 8 bits
        if(Column % 8 == 7)  ptr++;
...
sFONT *GUI_GetFontSize(POINT Dx, POINT Dy)
{
    ...
    if ((Dx > Font20.Width && Dx < Font24.Width) &&
        (Dy > Font20.Height && Dy < Font24.Height)) {
        Font = &Font20;
    } else 
    if ((Dx > Font16.Width && Dx < Font20.Width) &&
        (Dy > Font16.Height && Dy < Font20.Height)) {
        Font = &Font16;
    } else 
    if ((Dx > Font12.Width && Dx < Font16.Width) &&
        (Dy > Font12.Height && Dy < Font16.Height)) {
        Font = &Font12;
    } else 
    if ((Dx > Font8.Width && Dx < Font12.Width) &&
...
void GUI_Show(void)
{
  ...
  if(LCD_2_8==id){
    if (sLCD_DIS.LCD_Dis_Page > sLCD_DIS.LCD_Dis_Column)
    {
      GUI_Clear(WHITE);
			...
			GUI_DisString(10, 120, "WaveShare Electronic", &Font16, LCD_BACKGROUND, BLUE);
			GUI_DisString(40, 150, "2.8inch TFTLCD", &Font16, RED, BLUE);
      ...
      GUI_Clear(WHITE);
			...
			//DEBUG("Display String\r\n");
			GUI_DisString(55, 80, "WaveShare Electronic", &Font16, LCD_BACKGROUND, BLUE);
			GUI_DisString(85, 110, "2.8inch TFTLCD", &Font16, RED, BLUE);
...
#endif
  {
		...
		if(sLCD_DIS.LCD_Dis_Column > sLCD_DIS.LCD_Dis_Page) { 
			...
			//DEBUG("Display String\r\n");
			GUI_DisString(80, 80, "WaveShare Electronic", &Font24, LCD_BACKGROUND, BLUE);
			GUI_DisString(80, 120, "3.5inch TFTLCD", &Font20, RED, BLUE);
			...
			//DEBUG("Display String\r\n");
			GUI_DisString(40, 120, "WaveShare Electronic", &Font24, LCD_BACKGROUND, BLUE);
			GUI_DisString(40, 180, "3.5inch TFTLCD", &Font20, RED, BLUE);
			//DEBUG("Display Nummber\r\n");
